...
namespace CSharpMate.Core
{
    ...
    where T : ICodeFixCommandBase
    {
        ...
        public async Task Walker<TCustomWalker>(Solution solution, CancellationToken cancellationToken)
           where TCustomWalker : CLICSharpSyntaxWalkerBase
        {
            IEnumerable<Document> documents = solution.Projects.SelectMany(e => e.Documents);
            await Walker<TCustomWalker>(documents, cancellationToken);
        }
        public async Task Walker<TCustomWalker>(IEnumerable<Document> documents, CancellationToken cancellationToken)
          where TCustomWalker : CLICSharpSyntaxWalkerBase
        {
